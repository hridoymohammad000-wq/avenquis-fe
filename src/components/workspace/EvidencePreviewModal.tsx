...
import { X, Download, ShieldCheck, FileText, Lock, CheckCircle2, Calendar, User, Hash } from 'lucide-react';
...
export const EvidencePreviewModal: React.FC<EvidencePreviewModalProps> = ({
  ...
  const fileName = evidence?.fileName || docItem?.fileName || 'Audit_Evidence.pdf';
  const fileSize = evidence?.fileSize || docItem?.fileSize || '2.4 MB';
  const uploadedBy = evidence?.uploadedBy || docItem?.uploadedBy || 'Senior Audit Associate';
  const uploadedAt = evidence?.uploadedAt || docItem?.uploadedAt || '2026-08-30';
  const hash = evidence?.hash || docItem?.hash || 'sha256:4f9a8820c19a9bb823fe4100c8712399aa821092';
    ...
    <div className="fixed inset-0 bg-stone-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fadeIn">
      <div className="bg-white rounded-3xl border border-[#EBE6DD] max-w-3xl w-full flex flex-col max-h-[90vh] text-left shadow-2xl overflow-hidden">
        ...
        <div className="p-5 bg-[#FAF7F2] border-b border-[#EBE6DD] flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="flex items-center space-x-2">
                <h3 className="text-base font-bold text-[#1C1F1E] font-serif break-all">{fileName}</h3>
          ...
          <div className="flex items-center space-x-2">
            <button
              onClick={() => alert(`Downloading signed & verified copy of ${fileName}`)}
        ...
        <div className="p-6 overflow-y-auto space-y-5 bg-[#FCFAF6] flex-1">
          ...
          <div className="p-4 rounded-2xl bg-white border border-[#EBE6DD] flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-xs">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <div className="text-[10px] font-bold uppercase tracking-wider text-stone-400">ISA 230 Tamper-Evident SHA-256 Checksum</div>
                <div className="font-mono text-xs text-[#113227] font-semibold break-all">{hash}</div>
              </div>
            </div>
          ...
          <div className="p-6 rounded-2xl bg-white border border-[#E5DDD0] shadow-sm space-y-4">
            <div className="border-b border-stone-200 pb-3 flex items-center justify-between">
              <div>
                <span className="text-[10px] font-mono uppercase tracking-widest text-[#8A5A18] font-bold">AVENQUIS AUDIT REPOSITORY • VERIFIED ARTIFACT</span>
                <h4 className="text-sm font-bold text-stone-900 mt-0.5">{fileName}</h4>
              </div>
              <span className="text-[10px] text-stone-400 font-mono">Page 1 of 1 (Verified)</span>
            </div>

            {isExcel ? (
              <div className="space-y-3 font-sans text-xs">
                <div className="text-[11px] text-stone-500 font-medium">Spreadsheet Data Preview (Extract from Primary Ledger Reconciliation):</div>
                  ...
                  <table className="w-full text-left text-xs">
                    <thead className="bg-[#FAF7F2] text-stone-600 font-bold border-b border-stone-200 text-[10px] uppercase">
                      <tr>
                        ...
                        <th className="px-3 py-2 text-right">Balance per Books (BDT)</th>
                        <th className="px-3 py-2 text-right">Confirmation / Tested (BDT)</th>
                    ...
                    <tbody className="divide-y divide-stone-100 text-stone-700">
                      <tr>
                        ...
                        <td className="px-3 py-2">Sonali Bank Ltd. - CA A/C #0021</td>
                        <td className="px-3 py-2 text-right font-mono font-semibold">24,580,210.00</td>
                        <td className="px-3 py-2 text-right font-mono text-emerald-700 font-semibold">24,580,210.00</td>
                        <td className="px-3 py-2 text-right font-mono text-emerald-600 font-bold">0.00 (Nil)</td>
                      </tr>
                      <tr>
                        <td className="px-3 py-2 font-mono">GL-101-004</td>
                        <td className="px-3 py-2">Eastern Bank PLC - USD Margin A/C</td>
                        <td className="px-3 py-2 text-right font-mono font-semibold">18,340,900.00</td>
                        <td className="px-3 py-2 text-right font-mono text-emerald-700 font-semibold">18,340,900.00</td>
                        <td className="px-3 py-2 text-right font-mono text-emerald-600 font-bold">0.00 (Nil)</td>
                      </tr>
                      <tr>
                        <td className="px-3 py-2 font-mono">GL-101-009</td>
                        <td className="px-3 py-2">City Bank PLC - Escrow Account</td>
                        <td className="px-3 py-2 text-right font-mono font-semibold">9,200,500.00</td>
                        <td className="px-3 py-2 text-right font-mono text-emerald-700 font-semibold">9,200,500.00</td>
                        <td className="px-3 py-2 text-right font-mono text-emerald-600 font-bold">0.00 (Nil)</td>
                      </tr>
                    </tbody>
                  </table>
              ...
              <div className="p-4 rounded-xl bg-[#FAF8F5] border border-stone-200 text-xs text-stone-700 space-y-2">
                <div className="font-semibold text-stone-900">Archive Manifest:</div>
                <ul className="list-disc list-inside space-y-1 text-stone-600 font-mono text-[11px]">
                  ...
                  <li>Monthly_VAT_Return_Submissions_Mushak_9_1.pdf (Signed)</li>
                </ul>
              </div>
            ) : (
              <div className="space-y-3 text-xs text-stone-700 leading-relaxed font-serif">
                <p className="italic text-stone-500">
                  "This document serves as an immutable evidence record collected under ISA 500 / ISA 505 procedures. 
                  Digital signature and cryptographic verification have been validated against firm root certificates."
                </p>
                <div className="p-3 bg-[#FAF8F5] rounded-xl border border-stone-200 text-xs font-sans space-y-1.5">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-stone-800">{confidentiality}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-stone-500">Audit Assertion Addressed:</span>
                    <span className="font-semibold text-[#113227]">Existence, Rights &amp; Obligations, Accuracy</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-stone-500">Retention Requirement:</span>
                    <span className="font-semibold text-stone-800">7 Years (ICAB / FRC Mandate)</span>
                  </div>
                </div>
              </div>
            )}
