...
const CLIENT_OPTIONS = [
  { name: 'Apex Footwear & Polymer Ltd.', engagementCode: 'AUD-2026-081', contactName: 'Advocate Shamsul Alam', email: 'legal.apex@apexfootwear.com.bd', designation: 'Head of Legal Affairs' },
  { name: 'Orbit Textiles & Apparels Group', engagementCode: 'VAT-2026-019', contactName: 'Mohammad Rafiqul Islam', email: 'rafiqul.islam@orbittextiles-bd.com', designation: 'Chief Financial Officer' },
  { name: 'Novartis Healthcare Bangladesh', engagementCode: 'TAX-2026-044', contactName: 'Tanima Chowdhury, CMA', email: 'tanima.chowdhury@novartis.com.bd', designation: 'Head of Taxation & Treasury' },
  { name: 'Synapse Tech Solutions Pte.', engagementCode: 'ADV-2026-008', contactName: 'Adnan Samiul', email: 'adnan.samiul@synapsetech.sg', designation: 'VP Finance & Operations' },
  { name: 'Green Delta Insurance PLC', engagementCode: 'AUD-2026-092', contactName: 'Kazi Farhan Ahmed', email: 'farhan.kazi@greendelta.com.bd', designation: 'General Manager - Accounts' },
];
...
export const NewClientRequestModal: React.FC<NewClientRequestModalProps> = ({
  ...
  const [contactDesignation, setContactDesignation] = useState(CLIENT_OPTIONS[0].designation);
  ...
  const [reminderFrequency, setReminderFrequency] = useState<'Daily' | 'Every 3 Days' | 'Weekly'>('Every 3 Days');

  // Dynamic File Requirement Checklist
  const [fileRequirements, setFileRequirements] = useState<Omit<PBCFileRequirement, 'id'>[]>([
    { title: 'Signed External Confirmation / Requisition Schedule', format: 'Signed PDF Scan', mandatory: true, received: false },
    { title: 'Sub-ledger Trial Balance & Reconciliations', format: 'Excel Spreadsheet (.xlsx)', mandatory: true, received: false },
  ]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const formattedFiles: PBCFileRequirement[] = fileRequirements.map((fr, idx) => ({
      ...fr,
    ...
    if (onToast) {
      onToast(`PBC Request created & dispatch logged for ${chosenClient.name}.`, 'success');
    }
  ...
  return (
    <div className="fixed inset-0 bg-stone-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fadeIn text-left">
      <div className="bg-white rounded-3xl border border-[#EBE6DD] max-w-3xl w-full max-h-[92vh] flex flex-col overflow-hidden shadow-2xl">
        ...
        <div className="p-6 bg-[#FAF7F2] border-b border-[#EBE6DD] flex items-start justify-between">
          <div>
            ...
            <p className="text-xs text-stone-500 mt-0.5">
              Issue formal document and audit evidence requisitions directly to client management with automated reminder triggers.
            </p>
          </div>
        ...
        {/* Modal Body */}
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6 bg-[#FAF8F5]">
          ...
          <div className="bg-white p-5 rounded-2xl border border-[#EBE6DD] shadow-xs space-y-4">
            <div className="flex items-center justify-between">
              <h4 className="text-xs font-bold font-serif uppercase tracking-wider text-[#1C1F1E] flex items-center gap-1.5">
                ...
                <span>Deliverable Files Checklist ({fileRequirements.length})</span>
              </h4>
              <span className="text-[10px] text-stone-400 font-semibold">Client Upload Requirements</span>
            </div>
              ...
              {fileRequirements.map((fr, idx) => (
                ...
                >
                  <div className="space-y-0.5">
                    ...
                    <div className="text-[10px] font-mono text-stone-500">{fr.format}</div>
                  </div>
            ...
            <div className="p-3 rounded-xl bg-[#FAF0DE]/50 border border-[#EADBBF] space-y-2.5">
              <div className="text-[11px] font-bold text-[#8A5A18]">Add File Requirement</div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="Excel Spreadsheet (.xlsx)">Excel Spreadsheet (.xlsx)</option>
                    <option value="Signed PDF Scan">Signed PDF Scan</option>
                    <option value="Bank Certificate (.pdf)">Bank Certificate (.pdf)</option>
                    <option value="Archive (.zip / .rar)">Archive (.zip / .rar)</option>
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 text-xs">
                <div>
                  ...
                  >
                    ...
                    <option value="Every 3 Days">Every 3 Days (Recommended)</option>
                    <option value="Weekly">Weekly Digest</option>
                  </select>
                </div>

                <div className="p-3 bg-[#FAF8F5] rounded-xl border border-[#EBE6DD] text-[11px] text-stone-600 flex items-center">
                  Client contact ({contactEmail}) will receive professional automated dispatch emails referencing ticket #{`PBC-2026-${Math.floor(100 + Math.random() * 900)}`}.
                </div>
              </div>
            )}
          </div>
