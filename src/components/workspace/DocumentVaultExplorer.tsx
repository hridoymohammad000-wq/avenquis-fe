...
type SectionType = 'Current Audit File' | 'Permanent Audit File' | 'Tax Filings' | 'Legal Records';
...
export const DocumentVaultExplorer: React.FC<DocumentVaultExplorerProps> = ({
  ...
  const [selectedEngagementType, setSelectedEngagementType] = useState<string>('All');
  ...
  const [uploadClient, setUploadClient] = useState('Apex Footwear & Polymer Ltd.');
  const [uploadYear, setUploadYear] = useState('FY 2025-26');
  const [uploadEngagementType, setUploadEngagementType] = useState('Statutory Audit');
  const [uploadSection, setUploadSection] = useState<SectionType>('Current Audit File');
  const [uploadCategory, setUploadCategory] = useState<DocumentVaultItem['category']>('Trial Balance');
  const [uploadConfidentiality, setUploadConfidentiality] = useState<'Public' | 'Internal' | 'Highly Confidential'>('Highly Confidential');
  ...
  const engagementTypesList = useMemo(() => {
    return ['Statutory Audit', 'Tax Advisory', 'VAT Compliance', 'Corporate Advisory'];
  }, []);
  ...
  const filteredDocs = useMemo(() => {
    return documents.filter((doc) => {
      // Directory filtering
      if (selectedClient !== 'All' && doc.clientName !== selectedClient) return false;
      if (selectedYear !== 'All' && (doc.financialYear || 'FY 2025-26') !== selectedYear) return false;
      if (selectedEngagementType !== 'All' && (doc.engagementType || 'Statutory Audit') !== selectedEngagementType) return false;
      if (selectedSection !== 'All' && (doc.section || 'Current Audit File') !== selectedSection) return false;

      // Category & Confidentiality
      if (categoryFilter !== 'All' && doc.category !== categoryFilter) return false;
      if (confidentialityFilter !== 'All' && (doc.confidentiality || 'Internal') !== confidentialityFilter) return false;
      ...
      if (searchQuery) {
        ...
        if (!matchesName && !matchesClient && !matchesHash && !matchesCat) return false;
      }
  ...
  const handleUploadSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const finalName = uploadFileName.trim() || `Audit_Working_Doc_${Date.now()}.pdf`;
      ...
      setUploadProgress((prev) => {
        ...
        if (prev >= 90) {
          ...
          setTimeout(() => {
            ...
            setUploadFileName('');
            if (onToast) {
              onToast(`Document "${finalName}" encrypted and stored into ${uploadSection}.`, 'success');
            }
          }, 300);
          return 100;
        }
    ...
    <div className="space-y-6 text-left animate-fadeIn">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="p-4 rounded-2xl bg-white border border-[#EBE6DD] flex items-center space-x-3 shadow-xs">
          ...
          <div>
            <div className="text-[10px] font-bold uppercase tracking-wider text-stone-400">Total Vault Storage</div>
            <div className="text-lg font-bold font-serif text-[#1C1F1E]">{documents.length} Encrypted Files</div>
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-white border border-[#EBE6DD] flex items-center space-x-3 shadow-xs">
          ...
          <div>
            <div className="text-[10px] font-bold uppercase tracking-wider text-stone-400">High Confidentiality</div>
            <div className="text-lg font-bold font-serif text-[#8A5A18]">
              {documents.filter((d) => d.confidentiality === 'Highly Confidential').length} Restrictive Docs
            </div>
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-white border border-[#EBE6DD] flex items-center space-x-3 shadow-xs">
          ...
          <div>
            <div className="text-[10px] font-bold uppercase tracking-wider text-stone-400">Active Working Sections</div>
            <div className="text-lg font-bold font-serif text-[#1D526D]">4 Standard ISA Tiers</div>
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-white border border-[#EBE6DD] flex items-center space-x-3 shadow-xs">
          ...
          <div>
            <div className="text-[10px] font-bold uppercase tracking-wider text-stone-400">Tamper-Proof Lock Rate</div>
            <div className="text-lg font-bold font-serif text-[#1F5946]">100% SHA-256 Hashed</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 bg-white p-5 rounded-3xl border border-[#EBE6DD] shadow-xs space-y-4 flex flex-col">
          <div className="flex items-center justify-between border-b border-[#F0EBE1] pb-3">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-sm font-bold font-serif text-[#1C1F1E]">Directory Hierarchy</h3>
            </div>
            <button
              onClick={() => {
          ...
          <div className="text-xs text-stone-500 font-medium leading-tight">
            Structure: <span className="text-[#113227] font-semibold">Client Name → Financial Year → Engagement Type → Working Paper Section</span>
          </div>
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold uppercase tracking-wider text-stone-400 flex items-center justify-between">
              ...
              <span className="font-mono text-stone-500">{selectedClient === 'All' ? 'All Clients' : selectedClient}</span>
            </label>
            <div className="space-y-1 max-h-36 overflow-y-auto pr-1">
              ...
              {clientsList.map((client) => {
                const count = documents.filter((d) => d.clientName === client).length;
                return (
                  <button
                    key={client}
            ...
            <div className="grid grid-cols-2 gap-1.5">
              <button
                ...
                className={`px-2.5 py-1.5 rounded-xl text-xs font-semibold transition-all cursor-pointer ${
                  selectedYear === 'All' ? 'bg-[#FAF0DE] text-[#8A5A18] border border-[#EADBBF]' : 'bg-[#FAF8F5] text-stone-600 hover:bg-[#F2ECE1]'
                }`}
              >
                All FYs
              </button>
              {yearsList.map((yr) => (
                <button
                  ...
                  className={`px-2.5 py-1.5 rounded-xl text-xs font-semibold transition-all cursor-pointer ${
                    selectedYear === yr ? 'bg-[#FAF0DE] text-[#8A5A18] border border-[#EADBBF]' : 'bg-[#FAF8F5] text-stone-600 hover:bg-[#F2ECE1]'
                  }`}
                >
                  {yr}
                </button>
          ...
          <div className="space-y-1.5 pt-2 border-t border-[#F0EBE1]">
            <label className="text-[10px] font-bold uppercase tracking-wider text-stone-400 flex items-center justify-between">
              ...
              <span className="font-mono text-stone-500">{selectedEngagementType}</span>
            </label>
            <div className="grid grid-cols-2 gap-1.5">
              <button
                onClick={() => setSelectedEngagementType('All')}
                className={`px-2.5 py-1.5 rounded-xl text-xs font-semibold transition-all cursor-pointer ${
                  selectedEngagementType === 'All' ? 'bg-[#E1F3EE] text-[#113227] border border-[#BCE1D5]' : 'bg-[#FAF8F5] text-stone-600 hover:bg-[#F2ECE1]'
                }`}
              >
                All Services
              </button>
              {engagementTypesList.map((eng) => (
                <button
                  ...
                  className={`px-2.5 py-1.5 rounded-xl text-xs font-semibold transition-all cursor-pointer truncate ${
                    selectedEngagementType === eng ? 'bg-[#E1F3EE] text-[#113227] border border-[#BCE1D5]' : 'bg-[#FAF8F5] text-stone-600 hover:bg-[#F2ECE1]'
                  }`}
                >
                  {eng}
                </button>
          ...
          <div className="space-y-1.5 pt-2 border-t border-[#F0EBE1]">
            <label className="text-[10px] font-bold uppercase tracking-wider text-stone-400 flex items-center justify-between">
              ...
              <span className="font-mono text-stone-500">{selectedSection}</span>
            </label>
            <div className="space-y-1">
              <button
                ...
                className={`w-full text-left px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center space-x-2 transition-all cursor-pointer ${
                  selectedSection === 'All' ? 'bg-[#113227] text-white' : 'bg-[#FAF8F5] text-stone-700 hover:bg-[#F2ECE1]'
                }`}
              >
                <Folder className="w-3.5 h-3.5 shrink-0" />
              ...
              {sectionsList.map((sec) => {
                const count = documents.filter((d) => (d.section || 'Current Audit File') === sec).length;
                return (
                  <button
                    key={sec}
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="bg-white p-3.5 rounded-2xl border border-[#EBE6DD] flex items-center space-x-2 text-xs text-stone-600 font-medium overflow-x-auto shadow-xs">
            ...
            <span className={selectedClient !== 'All' ? 'font-bold text-[#113227]' : 'text-stone-500'}>
              {selectedClient === 'All' ? 'All Clients' : selectedClient}
            </span>
            <ChevronRight className="w-3.5 h-3.5 text-stone-400 shrink-0" />
            <span className={selectedYear !== 'All' ? 'font-bold text-[#113227]' : 'text-stone-500'}>
              {selectedYear === 'All' ? 'All FY' : selectedYear}
            </span>
            <ChevronRight className="w-3.5 h-3.5 text-stone-400 shrink-0" />
            <span className={selectedEngagementType !== 'All' ? 'font-bold text-[#113227]' : 'text-stone-500'}>
              {selectedEngagementType === 'All' ? 'All Services' : selectedEngagementType}
            </span>
            <ChevronRight className="w-3.5 h-3.5 text-stone-400 shrink-0" />
            <span className={selectedSection !== 'All' ? 'font-bold text-[#8A5A18] bg-[#FAF0DE] px-2 py-0.5 rounded' : 'text-stone-500'}>
          ...
          <div className="bg-white p-5 rounded-3xl border border-[#EBE6DD] shadow-xs space-y-4">
            <div className="flex items-center justify-between border-b border-[#F0EBE1] pb-3">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <h4 className="text-sm font-bold font-serif text-[#1C1F1E]">Secure File Upload Zone</h4>
                  <p className="text-[11px] text-stone-500">Drag &amp; drop with confidentiality tagging &amp; SHA-256 integrity sealing.</p>
                </div>
              </div>
            ...
            <form onSubmit={handleUploadSubmit} className="space-y-4">
              ...
              >
                ...
                <div className="text-xs font-bold text-[#1C1F1E]">
                  {uploadFileName ? (
                    ...
                    <span>Drag and drop audit file here, or <strong className="text-[#8A5A18] underline">browse documents</strong></span>
                  )}
                </div>
                <div className="text-[10px] text-stone-400 mt-1">
                  Supported formats: PDF, XLSX, DOCX, ZIP, CSV, XML (Max 50MB per file)
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-wider text-stone-500 mb-1">Target Client</label>
                  <select
                    value={uploadClient}
                ...
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-wider text-stone-500 mb-1">Working Paper Section</label>
                  <select
                    value={uploadSection}
                    onChange={(e) => setUploadSection(e.target.value as SectionType)}
                    className="w-full px-3 py-2 bg-[#FAF8F5] border border-[#E5DDD0] rounded-xl text-xs focus:outline-none focus:border-[#113227] font-medium"
                  >
                    {sectionsList.map((s) => (
                ...
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-wider text-stone-500 mb-1">Category Tag</label>
                  <select
                    value={uploadCategory}
                    onChange={(e) => setUploadCategory(e.target.value as any)}
                    className="w-full px-3 py-2 bg-[#FAF8F5] border border-[#E5DDD0] rounded-xl text-xs focus:outline-none focus:border-[#113227] font-medium"
                  >
                    <option value="Trial Balance">Trial Balance / Lead Schedule</option>
                    <option value="Bank Confirmation">Bank Confirmation (ISA 505)</option>
                    <option value="Engagement Letter">Engagement Letter &amp; Independence</option>
                    <option value="Board Minutes">Board Minutes &amp; Resolutions</option>
                    <option value="Tax Return">Tax Return / Challan</option>
                    <option value="Legal Certificate">Legal Certificate / RJSC</option>
                    <option value="Voucher Evidence">Voucher Evidence</option>
                  </select>
                </div>

                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-wider text-stone-500 mb-1">Confidentiality Label</label>
                  <select
                    value={uploadConfidentiality}
                    onChange={(e) => setUploadConfidentiality(e.target.value as any)}
                  ...
                  >
                    <option value="Highly Confidential">Highly Confidential (Restricted)</option>
                    <option value="Internal">Internal (Engagement Team Only)</option>
                ...
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-wider text-stone-500 mb-1">Engagement Service</label>
                  <select
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredDocs.length === 0 ? (
                <div className="col-span-2 p-12 text-center bg-white rounded-3xl border border-[#EBE6DD] space-y-3">
                  ...
                  <div className="text-sm font-bold text-stone-700">No documents found matching this directory path or filter</div>
                  <p className="text-xs text-stone-400">Try changing the directory folder on the left or clearing filters.</p>
                </div>
              ) : (
                filteredDocs.map((doc) => (
                  ...
                  >
                    <div>
                      ...
                      {/* File Name */}
                      <h4 className="text-xs font-bold text-[#1C1F1E] break-all leading-snug hover:text-[#113227] cursor-pointer"
                    ...
                    <div className="pt-3 mt-3 border-t border-[#F0EBE1] flex items-center justify-between text-xs text-[#66706B]">
                      <span className="text-[11px] font-mono">{doc.fileSize} • {doc.uploadedAt}</span>
