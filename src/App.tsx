...
export default function App() {
  ...
  const enterAdmin = async (me: Awaited<ReturnType<typeof authApi.me>>) => { await adminApi.overview(); const fullName = me.user.fullName || me.user.email.split('@')[0]; setSession({ id: me.user.id, name: fullName, email: me.user.email, role: me.user.platformRoles?.[0] || 'Platform staff', platformRoles: me.user.platformRoles, tenant: 'AVENQUIS Platform', initials: fullName.split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase(), avatarColor: '#102E25', mfaEnabled: me.user.mfaEnabled }); };
  useEffect(() => { const onPop = () => setPath(window.location.pathname || '/'); const onExpired = () => { setSession(null); setPath('/sign-in'); window.history.replaceState({}, '', '/sign-in'); }; setSessionExpiredHandler(onExpired); window.addEventListener('popstate', onPop); const bootstrap = async () => { try { const me = await authApi.me(); if (isAdminPath(window.location.pathname)) await enterAdmin(me); else await enterWorkspace(me.user.email, me.user.fullName); } catch (error) { if (isAdminPath(window.location.pathname)) { setSession(null); window.history.replaceState({}, '', '/'); setPath('/'); showToast(error instanceof Error ? error.message : 'Platform admin access is unavailable.', 'info'); } } finally { setBooting(false); } }; void bootstrap(); return () => { setSessionExpiredHandler(null); window.removeEventListener('popstate', onPop); }; }, []);
