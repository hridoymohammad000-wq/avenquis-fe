...
import { adminApi, authApi, tenantApi, BackendTenant, setSessionExpiredHandler, clearCsrfToken } from './lib/api';
import { bootstrapDestination, postAuthenticationDestination, publicMarketingPaths } from './lib/auth-routing';
...
import { AdminConsole, isAdminPath } from './components/admin/AdminConsole';
...
export default function App() {
  ...
  const enterWorkspace = async (me: AuthenticatedProfile) => { try { const memberships = await tenantApi.list(); if (!memberships.length) throw new Error('Your account is not assigned to an active practice.'); const first = memberships[0]; const context = await tenantApi.current(first.tenantId); setSession({ id: me.user.id, name: me.user.fullName || me.user.email.split('@')[0], email: me.user.email, role: 'Member', tenant: context.tenant.name, initials: (me.user.fullName || 'A').split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase(), avatarColor: '#113227', tenantId: first.tenantId, membershipId: context.membership.id, permissions: context.permissions, mfaEnabled: me.user.mfaEnabled }); setTenants(memberships.map((tenant: BackendTenant) => ({ id: tenant.tenantId, name: tenant.tenantName, location: '—', activeEngagements: 0 }))); showToast('Secure workspace loaded.'); } catch (error) { showToast(error instanceof Error ? error.message : 'Unable to load your workspace.', 'info'); } };
  const enterAdmin = async (me: AuthenticatedProfile) => { await adminApi.overview(); const fullName = me.user.fullName || me.user.email.split('@')[0]; setSession({ id: me.user.id, name: fullName, email: me.user.email, role: me.user.platformRoles?.[0] || 'Platform staff', platformRoles: me.user.platformRoles, tenant: 'AVENQUIS Platform', initials: fullName.split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase(), avatarColor: '#102E25', mfaEnabled: me.user.mfaEnabled }); };
  const completeSignIn = async (me: AuthenticatedProfile) => { if (postAuthenticationDestination(me.user) === 'admin') { await enterAdmin(me); navigate('/admin/overview'); return; } await enterWorkspace(me); };
  useEffect(() => { const onPop = () => setPath(window.location.pathname || '/'); const onExpired = () => { setSession(null); setPath('/sign-in'); window.history.replaceState({}, '', '/sign-in'); }; setSessionExpiredHandler(onExpired); window.addEventListener('popstate', onPop); const bootstrap = async () => { const currentPath = window.location.pathname; try { const me = await authApi.me(); const destination = bootstrapDestination(currentPath, me.user, isAdminPath(currentPath)); if (destination === 'admin') await enterAdmin(me); else if (destination === 'workspace') await enterWorkspace(me); } catch (error) { if (isAdminPath(currentPath)) { setSession(null); window.history.replaceState({}, '', '/'); setPath('/'); showToast(error instanceof Error ? error.message : 'Platform admin access is unavailable.', 'info'); } } finally { setBooting(false); } }; void bootstrap(); return () => { setSessionExpiredHandler(null); window.removeEventListener('popstate', onPop); }; }, []);
  ...
  if (session && isAdminPath(path)) return <AdminConsole path={path} session={session} navigate={navigate} onSignOut={async () => { try { await authApi.logout(); } catch (error) { showToast(error instanceof Error ? error.message : 'Unable to sign out from the server.', 'info'); return; } clearCsrfToken(); setSession(null); navigate('/'); showToast('Signed out of AVENQUIS Platform Admin.', 'info'); }} />;
...
function AuthPage({ path, navigate, onSignIn }: { path: string; navigate: (href: string) => void; onSignIn: (profile: AuthenticatedProfile) => Promise<void> }) {
