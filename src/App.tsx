...
import { shouldEnterAdmin } from './lib/auth-routing';
...
export default function App() {
  ...
  const completeSignIn = async (me: AuthenticatedProfile) => { if (shouldEnterAdmin(me.user)) { await enterAdmin(me); navigate('/admin/overview'); return; } await enterWorkspace(me); };
  useEffect(() => { const onPop = () => setPath(window.location.pathname || '/'); const onExpired = () => { setSession(null); setPath('/sign-in'); window.history.replaceState({}, '', '/sign-in'); }; setSessionExpiredHandler(onExpired); window.addEventListener('popstate', onPop); const bootstrap = async () => { try { const me = await authApi.me(); if (shouldEnterAdmin(me.user)) { await enterAdmin(me); if (!isAdminPath(window.location.pathname)) { window.history.replaceState({}, '', '/admin/overview'); setPath('/admin/overview'); } } else if (isAdminPath(window.location.pathname)) await enterAdmin(me); else await enterWorkspace(me); } catch (error) { if (isAdminPath(window.location.pathname)) { setSession(null); window.history.replaceState({}, '', '/'); setPath('/'); showToast(error instanceof Error ? error.message : 'Platform admin access is unavailable.', 'info'); } } finally { setBooting(false); } }; void bootstrap(); return () => { setSessionExpiredHandler(null); window.removeEventListener('popstate', onPop); }; }, []);
